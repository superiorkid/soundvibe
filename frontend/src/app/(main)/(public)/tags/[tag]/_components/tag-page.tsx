...
import { useLatestTracksByGenre } from "@/hooks/tanstack/genre";
...
const TagPage = ({ name }: TagPageProps) => {
  const { isError, isPending, refetch, tracks } = useLatestTracksByGenre({
    name,
    limit: 10,
  });
  ...
  if (!tracks?.data?.length) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          There are no latest tracks under{" "}
