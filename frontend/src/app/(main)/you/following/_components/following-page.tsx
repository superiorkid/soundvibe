...
import { useFollowing } from "@/hooks/tanstack/follows";
...
const FollowingPage = ({ userId }: FollowingPageProps) => {
  const { debouncedQuery } = useFollowingFilter();

  const { following, isError, isPending, refetch } = useFollowing({
    userId,
    filter: debouncedQuery as string,
  });
